...
export const WordService = {
    ...
    /**
     * Chèn Content Control tại vị trí con trỏ
     */
    insertContentControlAtSelection: async (tag, title) => {
        return await Word.run(async (context) => {
            ...
            cc.title = title || tag;
            cc.appearance = "BoundingBox";
            cc.color = "#4f46e5"; // Indigo color
            
            // Nếu selection trống, chèn văn bản gợi ý
            range.load("isEmpty");
            await context.sync();
            if (range.isEmpty) {
                cc.insertText(`[${title || tag}]`, "Replace");
            }

            // Lưu ý: ContentControl.insertText không hỗ trợ "After", nên phải lấy Range của nó trước.
            // cc.getRange() lấy phạm vi bao trùm toàn bộ Content Control
            const ccRange = cc.getRange();
            const spaceRange = ccRange.insertText(" ", "After");
            spaceRange.select("End");
    ...
    /**
     * Chèn Bookmark tại vị trí con trỏ
     */
    insertBookmarkAtSelection: async (name) => {
        return await Word.run(async (context) => {
            const range = context.document.getSelection();
    ...
    insertTableWithBookmark: async (bookmarkName, colCount, headers, noBorder = false) => {
        return await Word.run(async (context) => {
            ...
            if (headers && headers.length === colCount) {
                // Điền tiêu đề bằng phương thức getCell trực tiếp (an toàn, không cần load)
                for (let i = 0; i < colCount; i++) {
                    const cell = table.getCell(0, i);
                    cell.body.insertText(headers[i], "Replace");
                    cell.verticalAlignment = "Center";
    ...
    processExport: async (mode, tenDuAn, options = {}) => {
        ...
        if (mode === 'split') {
            ...
            // CHẾ ĐỘ NÂNG CAO: Tự động tìm các Bookmark bắt đầu bằng TT_ (Tờ trình) hoặc SPLIT_
            const dynamicBookmarks = availableBookmarks.filter(name => 
                name.startsWith("TT_") || name.startsWith("SPLIT_")
            );

            // Xây dựng danh sách các nhóm tách
            const splitGroups = [];
            
            // 1. Thêm các nhóm mặc định nếu có bookmark tương ứng
            const defaultMappings = [
                { folder: "1. To trinh Nhan su thi cong", bm: "TT_BCH", file: "0. QD thanh lap BCH.docx" },
                { folder: "1. To trinh Nhan su thi cong", bm: "TT_NhanSu", file: "1. To trinh Nhan su thi cong.docx" },
                { folder: "2. To trinh May moc thiet bi", bm: "TT_MayMoc", file: "0. To trinh May moc thiet bi.docx" },
                { folder: "3. To trinh Vat lieu su dung", bm: "TT_VatLieu", file: "0. To trinh Vat lieu su dung.docx" },
                { folder: "4. To trinh Ke hoach", bm: "TT_KeHoach", file: "0. To trinh Ke hoach.docx" },
                { folder: "5. To trinh Tien do Thi cong", bm: "TT_TienDoThiCong", file: "0. To trinh Tien do Thi cong.docx" },
                { folder: "6. To trinh Thi nghiem vat lieu", bm: "TT_ThiNghiem", file: "0. To trinh Thi nghiem vat lieu.docx" }
            ];

            defaultMappings.forEach(m => {
                if (availableBookmarks.includes(m.bm)) {
                    splitGroups.push({
                        folder: m.folder,
                        bookmarks: [m.bm],
                        fileName: m.file
            ...
            // 2. Thêm các bookmark động (do người dùng tự tạo qua Template Creator)
            // Lưu ý: Tránh trùng lặp với các bookmark mặc định đã xử lý ở trên
            dynamicBookmarks.forEach(bmName => {
                const isDefault = defaultMappings.some(m => m.bm === bmName);
                if (!isDefault) {
                    const stripped = bmName.replace(/^TT_|^SPLIT_/, "");

                    // Phương án A: Bookmark dạng TT_[thuMuc]__[tenFile]
                    // Dấu __ (double underscore) là phân cách giữa thư mục và tên file
                    if (stripped.includes("__")) {
                        const sepIdx = stripped.indexOf("__");
                        const folderPart = stripped.slice(0, sepIdx).replace(/_/g, " ");
                        const filePart = stripped.slice(sepIdx + 2).replace(/_/g, " ");
                        splitGroups.push({
                            folder: folderPart,
                            bookmarks: [bmName],
                            fileName: `${filePart}.docx`
                        });
                    } else {
                        // Legacy: bookmark cũ chỉ có tên file, không có thư mục
                        const cleanName = stripped.replace(/_/g, " ");
                        splitGroups.push({
                            folder: "Cac tai lieu khac",
                            bookmarks: [bmName],
            ...
            if (splitGroups.length === 0) {
                console.warn("Không tìm thấy bookmark nào để tách (TT_ hoặc SPLIT_)");
                if (onProgress) onProgress("⚠️ Không tìm thấy Bookmark để tách hồ sơ.", 100);
                return;
            }

            console.log(`Total detected split entries: ${splitGroups.length}`);
